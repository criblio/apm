...
2. **Lean on Cribl Search** — notifications use Cribl's built-in
   notification targets (Slack, email, PagerDuty, webhooks),
   configured at the product level and shared across Stream and
   Search. We reference existing targets by ID — never ask users
   to paste webhook URLs into our app. Each alert can target
   multiple destinations (the Cribl API accepts an array of
   target IDs per notification item).
...
Settings ("Auto-alert notification targets") configures where all
auto-alert notifications go by default — a multi-select picker
populated from `GET /api/v1/notification-targets`. Users can
select multiple destinations (e.g., Slack AND PagerDuty) so a
single alert reaches every relevant channel. Individual
auto-alerts can be overridden on the Alerts page (e.g., silence
a specific noisy service, or route a critical service's alerts
to a different escalation path).
...
- **From detected issue** — "Create alert from this issue" pre-fills
  the service + condition. The user picks notification targets
  (multi-select) and optionally adjusts the threshold.
...
Each user-created alert becomes a Cribl saved search with one or
more notification items. The saved search runs on the configured
cadence and fires through Cribl's notification infrastructure.
Multiple targets per alert are supported — the Cribl
`schedule.notifications.items[].targets` field accepts an array
of notification target IDs, and multiple `items` can be defined
per search for different conditions or routing rules.
...
interface AlertDefinition {
  ...
  notificationTargetIds: string[];  // Cribl notification target IDs (multi-select)
  enabled: boolean;
  createdAt: string;
...
7. **For state transitions that need notification:**
   ...
   - Dispatch to ALL configured notification targets for this alert
   - For auto-alerts: use the global target list from Settings,
     with per-alert overrides from KV if set
   - For user-created alerts: use the `notificationTargetIds` array
     from the alert definition
   - The same payload goes to every target (Slack gets the same
     message as PagerDuty); per-target formatting is handled by
     Cribl's notification target system
...
alerts that are Cribl saved searches fire independently via
Cribl's built-in `schedule.notifications` mechanism.

**Future: server-side evaluation.** If we want auto-alerts to fire
when nobody's looking, we need a scheduled search that reads state
from KV, evaluates conditions, updates state, and dispatches
...
| Notifications | Target names (comma-separated) or "none" |
| Actions | Edit / Delete / Silence (for user-created); Override targets / Silence (for auto) |
...
- Notification targets (multi-select picker, populated from
  `GET /api/v1/notification-targets` — shows all configured
  Slack channels, PagerDuty services, email lists, webhooks)
- Debounce settings (advanced, collapsed by default)

### Settings: Auto-alert notification targets

New section on the Settings page below "Detection cadence":

- **Auto-alert notification targets** — multi-select picker from
  the same `notification-targets` API. Controls where ALL
  auto-alerts send by default. Empty = auto-alerts are silent
  (detection only, no notifications).
- Clear label: "Auto-detected issues will notify these targets
  when they fire and when they resolve."
...
- Create Alert dialog with multi-target picker
- AlertDefinition CRUD in KV
- Notification target multi-select (reads from Cribl API)
- Auto-alert global targets setting in Settings page
...
## Cribl notification API reference

From `docs/research/cribl-saved-searches.md` — the API surface
we use for notification dispatch.

**List available targets:**
```
GET /api/v1/notification-targets
```
Returns all cross-product notification targets (Slack, PagerDuty,
email, webhooks, SNS, system_messages). These are configured by
the Cribl admin, not by our app.

**Saved search notification shape:**
```typescript
schedule.notifications.items[]: Array<{
  id: string;                    // e.g., "criblapm__alert_payment_1"
  condition: "search";
  targets: string[];             // array of target IDs — MULTI-TARGET
  conf: {
    triggerType: "resultsCount";
    triggerComparator: ">";
    triggerCount: number;        // 0 = fire when any results exist
    message: string;             // template with {{timestamp}}, {{searchResultsUrl}}, etc.
    savedQueryId: string;
  };
  targetConfigs?: Array<{
    id: string;                  // target ID
    conf: {
      ...
      attachmentType?: string;   // "inline" | "csv"
    };
  }>;
...
Key insight: `targets` is an array, so a single notification item
sends to multiple destinations simultaneously. We can also define
multiple `items` on the same saved search for different conditions
(e.g., one for firing, one for resolved — if the query output
distinguishes them).
